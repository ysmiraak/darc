...
class Setup(object):
    ...
    def cons(sents, form_w2v=None, lemm_w2v=None, proj=False):
        ...
        for idx, tran in enumerate(idx2tran):
            ...
            tran2idx[tran] = hotv
        data = [],     [],     [],     [],     [],     [],    []
        name = "form", "lemm", "upos", "drel", "feat", "mstr"
        form_append, lemm_append, upos_append, drel_append, feat_append, mstr_append \
    ...
    def model(self,
        ...
        mstr = Input(name="mstr", shape=self.x["mstr"].shape[1:], dtype=np.float32)
        # cons layers
        i = [upos, drel]
    ...
    def feature(self, config, named=True):
        ...
        # use results from pavel's mstnn
        mstr = np.zeros(3, np.float32)
        if 2 <= len(s):
            i = s[-1]
            j = s[-2]
            head = config.sent.head
            if head[i] == j:
                mstr[0] = 1.0
            elif i == head[j]:
                mstr[1] = 1.0
            ...
            mstr[2] = 1.0
        if named:
            return {'form': form, 'lemm': lemm, 'upos': upos, 'drel': drel, 'feat': feat, 'mstr': mstr}
        else:
            return [form, lemm, upos, drel, feat, mstr]
