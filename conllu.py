...
class AttrVals(AssocTuple):
    ...
    def parse(s):
        """-> AttrVals"""
        if "_" == s: return AttrVals()
        avs = []
        for p in s.split("|"):
            av = p.split("=")
            if 1 == len(av): av = p.split(":")
            if 2 == len(av): avs.append(av)
            elif 2 < len(av):
                for a in av[:-1]:
                    avs.append([a, av[-1]])
            elif avs:
...
class Token(object):
    ...
    def validate(self):
        ...
        for a in ('form','lemma','upostag','xpostag','deprel','deps'):
            ...
            if not isinstance(v, str): res.append(Fault(a, v, 'str'))
            if "" == v: res.append(Fault(a, v, "non-empty"))
        for a in ('feats','misc'):
            v = self.__getattribute__(a)
            if not isinstance(v, AttrVals): res.append(Fault(a, v, 'AttrVals'))
        return res

    def __init__(self, form="_", lemma="_", upostag="_", xpostag="_",
                 feats=AttrVals(), head="_", deprel="_", deps="_", misc=AttrVals()):
        self.form = form
        self.lemma = lemma
    ...
    def parse(s):
        """str -> Word | MultiWord"""
        args = s.split("\t")
        if len(args) != 10: raise TypeError("expected 10-col tsv, got: {}".format(s))
...
class Word(Token):
    ...
    def validate(self):
        res = []
        if not is_pos_int(self.id): res.append(Fault('id', self.id, "pos-int"))
        if not is_nat_int(self.head): res.append(Fault('head', self.head, "nat-int"))
...
class MultiWord(Token):
    ...
    def validate(self):
        res = []
        if not is_pos_int(self.lo) or not is_pos_int(self.hi) or self.lo > self.hi:
            res.append(Fault('id', "{}-{}".format(self.lo, self.hi), "invalid"))
        if "_" != self.head: res.append(Fault('head', self.head, "'_'"))
        res.extend(Token.validate(self))
        return res
...
class Sent(object):
    ...
    def validate(self):
        ...
        for m in self.multi:
            ...
            if hi >= m.lo: res.append(Fault('MultiWord', "spans", "non-overlapping"))
            if m.hi > len(self.words):
                res.append(Fault("MultiWord span", "{}-{}".format(m.lo, m.hi), "within bounds"))
            lo, hi = m.lo, m.hi
        return res
    ...
    def __init__(self, tokens):
        ...
        for t in tokens:
            if isinstance(t, Word):
                ...
                raise TypeError("expected Word or MultiWord, got {}: {}".format(type(t), t))
