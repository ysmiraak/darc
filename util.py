...
class AssocTuple(tuple):
    ...
    def __new__(cls, iterable=(), unsafe=False):
        if unsafe:
            return super(AssocTuple, cls).__new__(cls, iterable)
        if not iterable:
        ...
        for kv in iterable:
            if not hasattr(kv, '__len__') or 2 != len(kv):
                raise TypeError(
                    "AssocTuple expected pairs, got: {}".format(iterable))
            elif kv[0] in ks:
                raise TypeError(
                    "duplicate keys for AssocTuple: {}".format(iterable))
            else:
                ks.add(kv[0])
                kvs.append(tuple(kv))
    ...
    def __contains__(self, k):
        return AssocTuple.index(self, k) is not None
